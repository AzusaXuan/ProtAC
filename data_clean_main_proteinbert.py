...
from models.protac_pretrain_protein_v5 import protac_pretrain_seq_anno_mlm, protac_pretrain_v5_kw_pb
import utils
from utils import warmup_lr_schedule_k, step_lr_schedule_k
import subprocess
...
from dataloader import Dataset_pretrain_new, Dataset_swiss_random_test, Dataset_swiss_testset_kw_pb, create_loader, \
    create_sampler, Dataset_swiss_random, create_caption_loader
from torch.cuda.amp import autocast
from dataloader import Dataset_caption
import numpy as np
...
def train(model, optimizer, epoch, dataloader, args, test_dataloader):
    ...
    for i, batch in enumerate(dataloader):
        ...
        loss_ita, loss_itm, loss_lm_anno, loss_lm_seq, loss_lm_seq_mlm, anno_diff, itm_accuracy = outputs
        anno_accuracy, anno_precision,anno_recall, anno_fmax, anno_auc, seq_difference, seq_difference_mlm = anno_diff
        loss = loss_ita * args.itc_loss_weight + loss_itm * args.itm_loss_weight + loss_lm_anno * args.anno_lm_loss_weight + loss_lm_seq * args.seq_lm_loss_weight + loss_lm_seq_mlm * args.seq_lm_loss_weight
        ...
        if args.rank == 0 and args.use_wandb:
            ...
            if args.version.endswith("slm"):
                additional_metrics.update({
                    'LM_seq_loss': loss_lm_seq.item(),
                    'seq_difference': seq_difference.item(),
                    'LM_seq_loss_mlm': loss_lm_seq_mlm.item(),
                    'seq_difference_mlm': seq_difference_mlm.item(),
                })
        ...
        if i / total_batch > sav_cnt and args.rank == 0 and args.mode == "train":
            ...
            pretrain_checkpoint_i = os.path.join(args.output_dir,
                                                 f'{args.dataset}_{args.version}_clean_before_ft_depth_%d_heads_%d_bs_%d_e_%02d_per_%.2f.pth' % (
                                                     args.depth, args.attn_heads, args.batch_size, args.actual_epoch,
                                                     sav_cnt))
    ...
    if args.mode == "finetune":
        ...
        if args.rank == 0 and (epoch + 1) % 10 == 0 and (epoch + 1) != args.finetune_epoch:
            ...
            finetune_checkpoint_i = os.path.join(args.output_dir,
                                                 f'{args.version}_clean_after_ft_depth_%d_heads_%d_bs_%d_e_%02d_ft_%02d.pth' % (
                                                     args.depth, args.attn_heads, args.batch_size, args.actual_epoch,
                                                     epoch + 1))
        ...
        with tqdm(dataloader, dynamic_ncols=True, leave=False) as tqdmDataLoader:
            for i, batch in enumerate(tqdmDataLoader):
                ...
                if args.version.endswith("slm"):
                    loss_ita, loss_itm, loss_lm_anno, loss_lm_seq, loss_lm_seq_mlm, anno_diff, itm_accuracy = outputs
                    anno_accuracy, anno_precision, anno_recall, anno_fmax, anno_auc, seq_difference, seq_difference_mlm = anno_diff
                    loss = loss_ita * args.itc_loss_weight + loss_itm * args.itm_loss_weight + loss_lm_anno * args.anno_lm_loss_weight + loss_lm_seq * args.anno_lm_loss_weight + loss_lm_seq_mlm * args.anno_lm_loss_weight
                    metrics['TEST_LM_seq_loss'] += loss_lm_seq.item()
                    metrics['TEST_LM_seq_loss_mlm'] += loss_lm_seq_mlm.item()
                    metrics['TEST_seq_difference'] += seq_difference.item()

                else:
                    loss_ita, loss_itm, loss_lm_anno, anno_diff, itm_accuracy = outputs
                    anno_accuracy, anno_precision, anno_recall, anno_fmax, anno_auc = anno_diff
                    loss = loss_ita * args.itc_loss_weight + loss_itm * args.itm_loss_weight + loss_lm_anno * args.anno_lm_loss_weight
...
@torch.compile
def data_to_caption(model, dataloader, epoch, args, save_dir):  # 4GPU 2H20MIN
    ...
    with torch.no_grad():
        ...
        with tqdm(dataloader, dynamic_ncols=True, leave=False) as tqdmDataLoader:
            for i, batch in enumerate(tqdmDataLoader):
                ...
                input_seq = seqs.to(args.gpu)
                input_anno = kws.float().to(args.gpu)

                seq_hidden_embeds = seq_encoder(input_seq)
                decoder_output = anno_decoder(input_anno, seq_embed=seq_hidden_embeds)
                predict_anno = to_annotation_logits(decoder_output)
                anno_hat = torch.clamp(predict_anno, 0, 1).to(input_anno.device)

                output_pos_true = anno_encoder(input_anno, seq_embed=seq_hidden_embeds)
                output_pos_hat = anno_encoder(anno_hat, seq_embed=seq_hidden_embeds)
                ...
                new_kw = torch.where(mask.unsqueeze(1), anno_hat, input_anno)

                inds_np = inds.detach().cpu().numpy().astype(np.int32)
                seqs_np = seqs.detach().cpu().numpy().astype(np.int8)
                ...
                with h5py.File(output_file, 'a') as f:
                    ...
                    f.create_dataset(seq_name, data=seqs_np)

                    # Since the number of True positions may be different, use a special HDF5 data type
                    dt = h5py.special_dtype(vlen=np.dtype('int32'))
                    kws_dataset = f.create_dataset(kw_name, (input_anno.shape[0],), dtype=dt)

                    # Store the indices of True positions
                    kws_dataset[0:input_anno.shape[0]] = true_indices_per_row_chunk

                    f.flush()

                save_to_disk(f'caption_list_{epoch + 1}.npy', caption_list, append=True if chunk_counter > 0 else False)
        ...
        if caption_list:
            save_to_disk('caption_list.npy', caption_list, append=True if chunk_counter > 0 else False)
        print(f"Processed {total_data_len} data points. Captioned {new_ind} seqs.")


@torch.compile
def data_to_caption_new(model, dataloader, epoch, args, save_dir):
    ...
    with torch.no_grad():
        ...
        with tqdm(dataloader, dynamic_ncols=True, leave=False) as tqdmDataLoader:
            for index, batch in enumerate(tqdmDataLoader):
                inds, input_seq, input_anno = batch
                input_seq = input_seq.to(args.gpu)
                input_anno = input_anno.float().to(args.gpu)
                inds = inds.to(args.gpu)
                ...
                inds_np = inds.detach().cpu().numpy().astype(np.int32)
                new_kw_np = new_kw.detach().cpu().numpy().astype(np.bool_)  # new_kw: binary, 0,1
                seqs_np = input_seq.detach().cpu().numpy().astype(np.int8)
                # get kws
                non_zero = new_kw_np
                col_indices = np.argwhere(non_zero)
                true_indices_per_row_chunk = np.split(col_indices[:, 1], np.cumsum(np.sum(non_zero, axis=1))[:-1])
                ...
                if caption_list:
                    with h5py.File(caption_list_file, 'a') as f:
                        ...
                        if 'caption_inds' not in f:
                            ...
                            # maxshape = (None,) + (1,)  # Assume the array to be appended is 1D
...
def main(args):
    ...
    if args.version == "protac_noitc_noitm_alm_slm":
        args.use_itc = False
        args.use_itm = False
        args.use_alm = True
        args.use_slm = True
    ...
    if args.rank == 0 and args.use_wandb:
        wandb.init(project=args.wandb_project_name,
                   name="PB_"+args.dataset + "_" + args.mode + "_e_" + str(args.actual_epoch) + "_kw_" + str(
                       args.num_annotation) + "_" + args.version + '_bs_%d_dp_%d_h_%d' % (
                            args.batch_size, args.depth, args.attn_heads),
                   entity='protac-cleaning',
                   )

    total_epoch = args.actual_epoch

    args.init_checkpoint_path = f"model_{args.version}_layer{args.depth}_head{args.attn_heads}_init_PB_tiny.pth"
    args.init_checkpoint_path_kw = f"model_{args.version}_layer{args.depth}_head{args.attn_heads}_init_PB_tiny_kw.pth"
    pretrain_checkpoint = os.path.join(args.output_dir,
                                       f'{args.dataset}_{args.version}_clean_before_ft_depth_%d_heads_%d_bs_%d_e_%02d.pth' % (
                                           args.depth, args.attn_heads, args.batch_size, total_epoch))
    pretrain_sub_checkpoint = os.path.join(args.output_dir,
                                           f'{args.dataset}_{args.version}_clean_before_ft_depth_%d_heads_%d_bs_%d_e_%02d' % (
                                               args.depth, args.attn_heads, args.batch_size,
                                               total_epoch)) + '_sub_{}.pth'
    ...
    finetune_checkpoint = os.path.join(args.output_dir,
                                       f'{args.dataset}_{args.version}_clean_after_ft_depth_%d_heads_%d_bs_%d_e_%02d.pth' % (
                                           args.depth, args.attn_heads, args.batch_size, total_epoch))
    ...
    fewshot_checkpoint = os.path.join(args.output_dir,
                                      f'{args.dataset}_{args.version}_kw_pred_depth_%d_heads_%d_bs_%d_e_%02d.pth' % (
                                          args.depth, args.attn_heads, args.batch_size, total_epoch))

    test_dataset = Dataset_swiss_random_test()
    ...
    if args.mode == "train":  # Dataset_pretrain_new (tokenized)

        pretrain_dataset = Dataset_pretrain_new(
            dataset=args.dataset, seq_len=args.seq_len,
            num_annotation=args.num_annotation, epoch=args.actual_epoch, # args.actual_epoch
            captioned_seq_sav_dir=os.path.join(args.captioned_seq_sav_dir,
                                            f'{args.version}_split_sample_{args.depth}_{args.attn_heads}_epoch{total_epoch}.hdf5'),
            args=args)
    ...
    if args.mode == "finetune":
        train_dataset = Dataset_swiss_random(args.num_annotation, split="train")
    ...
    if args.mode == "caption":
        ...
        caption(model, args, total_epoch)
    ...
    if args.mode == "kw_pred":
        train_dataset = Dataset_swiss_testset_kw_pb(args.seq_len, split="train")
        train_sampler = create_sampler(train_dataset, True, args.world_size, args.rank)
        train_dataloader = \
            create_loader(train_dataset, train_sampler, batch_size=args.batch_size, num_workers=8, is_training=True)

        test_dataset = Dataset_swiss_testset_kw_pb(args.seq_len, split="test")
        ...
        args.checkpoint = pretrain_checkpoint if args.checkpoint is None else args.checkpoint
        model, model_without_ddp, optimizer_pretrain = create_model_kw(args, args.checkpoint)
    ...
    if args.mode == "caption_sw":
        ...
        caption_sw(model, args, total_epoch)
...
def caption(model, args, epoch):
    pre_dir = os.path.join(args.captioned_seq_sav_dir,
                           f"{args.version}_split_sample_{args.depth}_{args.attn_heads}_epoch{epoch}.hdf5")
    save_dir = os.path.join(args.captioned_seq_sav_dir,
                            f"{args.version}_split_sample_{args.depth}_{args.attn_heads}_epoch{epoch + 1}")
    inds_dir = os.path.join(args.captioned_ind_sav_dir,
                            f"caption_inds_epoch{epoch}")
    caption_dataset = Dataset_caption(captioned_seq_sav_dir=pre_dir, epoch=epoch, )
    caption_samplers = create_sampler(caption_dataset, False, args.world_size, args.rank)
    caption_dataloader = create_loader(caption_dataset, caption_samplers, args.sample_batch_size, 8, is_training=False)
    ...
    print("Start caption")
    start_time = time.time()
    data_to_caption_new(model, caption_dataloader, epoch, args, save_dir)
    if args.distributed:
        dist.barrier()
...
def caption_sw(model, args, epoch):
    ...
    pre_dir = 'path/to/swissprot_new_release.csv'
    df = pd.read_csv(pre_dir)
    print("Start caption")
    start_time = time.time()

    # data_to_caption(model, caption_dataloader, epoch, args, save_dir)
    seq_encoder = get_module_by_name(model, "seq_encoder")
    anno_encoder = get_module_by_name(model, "anno_encoder")
    anno_decoder = get_module_by_name(model, "anno_decoder")
    to_annotation_logits = get_module_by_name(model, "to_annotation_logits")
    ...
    with open(f'sw_cleaned_vs_updated_go_tiny_epoch{args.actual_epoch}_pretrain.csv', 'w', newline='') as csvfile:
        ...
        with torch.no_grad():
            for index, row in tqdm(df.iterrows()):
                ...
                tokenized_seq = [letter_to_num[a] for a in seqs]
                seq_length = len(tokenized_seq)
                encoded_seq = [tokenized_seq + (args.seq_len - seq_length) * [
                    pad_token_index]] if seq_length <= args.seq_len else [
                    tokenized_seq[:args.seq_len]]
                input_seq = torch.tensor(encoded_seq, dtype=torch.long).to(args.gpu)
                ...
                # print("go num: ", len(new_anno_list))
...
# Define Kaiming initialization function
def kaiming_init(m):
    if isinstance(m, (nn.Linear, nn.Conv1d)):
        # Use He initialization method to reset weights
        torch.nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
        # If there is a bias term, also reset the bias
        if m.bias is not None:
            nn.init.constant_(m.bias, 0)

def create_model(args, checkpoint=None):

    model = protac_pretrain_seq_anno_mlm(args=args)
    print('Successfully loading ProtAC-V5 model')
    ...
    print("seq encoder param: ", sum(p.numel() for p in model.seq_encoder.parameters()))
...
def create_model_kw(args, checkpoint=None):
    print("Creating model")
    model = protac_pretrain_v5_kw_pb(args=args)
    print('Successfully loading ProtAC-V5-kw model')
...
if __name__ == '__main__':
    ...
    ## model settings
    parser.add_argument('--version', default="protac_noitc_noitm_alm_slm", type=str,
                        help='[protac_noitc_noitm_alm_slm]')
    ...
    parser.add_argument('--attn_heads', default=4, type=int)  # 35m: 8
    parser.add_argument('--depth', default=6, type=int)  # 35m: 12
    ...
    parser.add_argument('--num_tokens', default=26, type=int)
    parser.add_argument('--exclude_token_ids', default=[20], type=list)
    ...
    parser.add_argument('--warmup_lr', default=1e-6, type=float) # 1e-6
    parser.add_argument('--weight_decay', default=0.05, type=float) # 0.05
    parser.add_argument('--init_lr', default=2e-5, type=float) # 2e-5
    parser.add_argument('--min_lr', default=1e-6, type=float) # 1e-6
    parser.add_argument('--lr_decay_rate', default=0.9, type=float) # 0.9
