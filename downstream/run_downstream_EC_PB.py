...
def parse_args():
    ...
    parser.add_argument("-c", "--config", help="yaml configuration file",
                        default="/configs/EC_pb_tiny.yaml")
...
def build_solver(cfg, logger):
    ...
    if cfg.get("checkpoint") is not None:
        ...
        for key in list(model_state_dict.keys()):
            if key.startswith('model.seq_encoder'):
                new_key = key.replace('model.seq_encoder', 'seq_encoder')
                if new_key in seq_encoder_items:
                    model_state_dict[key] = seq_encoder_items[new_key]
                    print(f"replace {new_key}")

        solver.model.load_state_dict(model_state_dict, strict=True)
